class Menu extends Phaser.Scene {
    ...
    create() {
        ...
        borderUISize - borderPadding, 'ROCKET PATROL', menuConfig).setOrigin(0.5);
        this.add.text(game.config.width/2, game.config.height/2, 
        'Use ⟷ arrows to move & (F) to fire', menuConfig).setOrigin(0.5);
        menuConfig.backgroundColor = '#8080AF';
        menuConfig.color = '#000';
        this.add.text(game.config.width/2, game.config.height/2 + 
        borderUISize + borderPadding, 'Press ⭠ for Novice or ➝ for Expert',
